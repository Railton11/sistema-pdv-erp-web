...
if($forma_pgto_input != ""){
	...
	$res = $pdo->prepare("INSERT INTO vendas SET valor = :valor, data = curDate(), hora = curTime(),  operador = :usuario, valor_recebido = :valor_recebido, desconto = :desconto, troco = :troco, forma_pgt = :forma_pgt, abertura = :abertura, status = 'Concluída' ");
	...
	$res->bindValue(":forma_pgt", $forma_pgto_input);
...
if(@count($res) > 0){
    ...
    if($estoque < $quantidade){
        echo "Quantidade no estoque insuficiente!&-/z Por enquanto temos ".$estoque ." de ".$nome;
        exit();
    }
...
$dados = $novo_estoque .'&-/z'. $nome .'&-/z'. 
    $descricao .'&-/z'. $imagem .'&-/z'. 
    $valor .'&-/z'. $valor_total .'&-/z'. 
    $valor_totalF .'&-/z'. $total_venda .'&-/z'. 
    $total_vendaF .'&-/z'. $troco .'&-/z'. $trocoF;
echo $dados;
