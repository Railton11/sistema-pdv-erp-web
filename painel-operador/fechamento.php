...
$total_reg = @count($res);
if($total_reg > 0){ 
	echo 'Existe uma venda em andamento, exclua os itens da venda ou finalize a venda para fechar o caixa!';
	exit();
}
...
$valor_quebra = $valor_fec - ($valor_abertura + $valor_vendido);
...
$res = $pdo->prepare("UPDATE caixa SET data_fec = curDate(), hora_fec = curTime(), valor_fec = :valor_fec, gerente_fec = :gerente_fec, valor_vendido = :valor_vendido, valor_quebra = :valor_quebra, status = 'Fechado' WHERE operador = '$id_usuario' and status = 'Aberto'");
